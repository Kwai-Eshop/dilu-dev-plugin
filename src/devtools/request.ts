...
export abstract class Intercepted implements InterceptedData {
  ...
  async getResponseBody() {
    ...
    const body = await this.getResponseBodyInternal();
  ...
  protected abstract continueRequestInternal(
    request: Partial<Debugger_Network_Request>
  ): Promise<void>;
  ...
  async continueRequest({
    ...
    requestHeaders,
    requestBody,
  }: Partial<InterceptedData>): Promise<void> {
  ...
  protected abstract continueResponseInternal(
    request: Partial<InterceptedData>
  ): Promise<void>;
  ...
  protected getModifiedRequest(
    obj: Partial<InterceptedData>
  ): Partial<Debugger_Network_Request> {
    const modified: Partial<Debugger_Network_Request> = {};
    ...
    if (
      ...
      (JSON.stringify(obj.requestHeaders) !==
        JSON.stringify(this.requestHeaders) ||
        modified.postData)
    ) {
      modified.headers = obj.requestHeaders.reduce(
        (ret: any, header: { name: any; value: any }) => ({
  ...
  protected async getModifiedResponse(
    obj: Partial<InterceptedData>
  ): Promise<Partial<InterceptedData>> {
    const modified: Partial<InterceptedData> = {};
    if (
...
export class FetchIntercepted extends Intercepted {
  ...
  async getResponseBodyInternal() {
    return this.debuggee.sendCommand("Fetch.getResponseBody", {
      requestId: this.id,
    });
  }
  ...
  async continueRequestInternal(request: Partial<Debugger_Network_Request>) {
    if (JSON.stringify(request) === "{}") {
      return this.debuggee.sendCommand("Fetch.continueRequest", {
        requestId: this.id,
      });
    }
    return this.debuggee.sendCommand("Fetch.continueRequest", {
