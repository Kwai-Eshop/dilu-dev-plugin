...
export class Debuggee {
  ...
  async waitForDetach(): Promise<string> {
    const detachReason = await new Promise((res: (reason: string) => void) => {
      chrome.debugger.onDetach.addListener((source, _detachReason: string) => {
  ...
  async sendCommand(
    ...
    params: { interceptionId: string }
  ): Promise<Debugger_Network_ResponseBody>;
  ...
  async sendCommand(
    ...
    params: { patterns: Debugger_Network_RequestPattern[] }
  ): Promise<never>;
  ...
  async sendCommand(
    method: "Fetch.enable",
    params: { patterns: Debugger_Fetch_RequestPattern[] }
  ): Promise<never>;
  ...
  async sendCommand(
    ...
    params: { requestId: string } & Partial<{
      url: string;
      method: string;
      postData: string;
      headers: Debugger_Fetch_HeaderEntry[];
  ...
  on(
    ...
    listener: (ret: Debugger_Network_requestIntercepted) => void
  ): void;

  on(
    event: "Fetch.requestPaused",
    listener: (ret: Debugger_Fetch_requestPaused) => void
  ): void;
